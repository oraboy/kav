...
**The tease is the hook, and the hook is the product.** Ending a chapter on a name nobody has explained is a strong move, and it stays strong for as long as the reader keeps turning pages on it. Some of those names should never be explained at all.

**What decides it is alignment.** Confusion the reader *shares with* the protagonist is nearly free, because it is the POV doing its job: ברקו can't follow the adults, so neither can the reader, and that is the book. Confusion *about* the protagonist is the expensive kind — why did he say that, what did she just do — because the reader is locked out of the one thing they came for. Same amount of not-knowing, opposite cost. And below both sits the kind nobody ever enjoys: not knowing who spoke or where the scene is, which is craft, not story.

So the question is never whether the reader knows. It is **whose understanding is missing, and for how long.** Track it in `stories/<slug>/reader-ledger.md`; the ladder and the duration rule are in `docs/know-how/cold-read.md` §3. A story that means to withhold something about its protagonist can do that, and does it by writing it into `story.md` **Declared unknowns** before the chapter is drawn, not by explaining afterwards that it was intentional.
