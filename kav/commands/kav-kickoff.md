...
- **Apparent age is a hard trait.** Write it in years. A mother drawn young in some panels reads as a sister, and a reader who has the wrong family builds the rest of the book on it.
...
Each place goes through **`/kav-location <name>`**, which does the intake below and shows a test scene. Interview or import, per the template: what it is · **whose turf** (every location has an owner; power tilts toward them there) · dramatic affordances · visual notes · reference images · history. **Register each location** as it lands, under one name for sheet, photo folder and registry: photos into `locations/<name>/`, an entry (photos + written English description) under `locations` in `briefs.json`, and trigger words under `location_words`. **Order matters**: the parser takes the first match, so a more specific place (`shop-entrance`) is listed before a general one (`shop`), and generic words ("apartment", "room") stay out of the trigger list — a generic word steals scenes meant for another place. **GATE on the set.**

**The reference set is the location's scene vocabulary — it decides what can be drawn there.** Collect to a shot list rather than to "the place": what will this chapter actually stage? Then record in the location file a small table of *which scene types this place can carry and which photos each one takes*, and what it cannot stage at all because nobody photographed it. That table is the real spec, and it pays twice — it tells the storyboard which scenes are available here, and it tells panel generation exactly which references to bind. A place photographed only as a façade can host only façade scenes, however much the story wants a conversation inside it.

**Never send a photo of the outside into a scene set inside,** or the reverse. Contradictory references are a more common cause of a location coming back as somewhere else than having too few — see the reference budget in `docs/know-how/image-prompting.md`.
...
**Seed the reader ledger** once the arc is approved: `stories/<slug>/reader-ledger.md` from the template, one OPEN row for every name, pre-story event, world rule and relationship the arc leans on — and, first, **every principal's name and relationship to the protagonist**, and **every term the story coins** that a later chapter will spend. Those two were the costliest gaps measured on a real book. This is the list of things the *reader* has to be told, as opposed to the things the book knows — nothing on it is paid yet, and chapters close the rows as they go (`docs/know-how/cold-read.md` §1). Say plainly which chapter is meant to pay each one; a fact the arc never pays is a hole worth finding now rather than in chapter 4.
